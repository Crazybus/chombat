...
describe('State Consistency', () => {
  ...
  const techs = {
    ...
    435: {
      ...
      name: 'Bloodlines',
      building: 101,
      effects: [{ type: 4, attribute: 0, value: 20, unitId: -1, class: 47 }],
      age: 2,
    },
  };
  ...
  it('should be idempotent when switching between ages', () => {
    // 1. Dark Age
    let config: ArmyState = { age: '1', bonuses: [] };
  ...
  it('should not accumulate manual overrides', () => {
    ...
    let config: ArmyState = { age: '1', overrides: { hp: 100 }, bonuses: [] };
    let sim = new CombatSim(baseScout, baseScout, config, config, techs as any, { '448': baseScout });
  ...
  it('should apply Bloodlines correctly every time', () => {
    const config: ArmyState = { age: '2', bonuses: [{ id: '435', effects: [true] }] };
