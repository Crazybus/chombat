...
const Layout: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    const scenarioName = cleanState.name || 'New Scenario';
    // Generate stable internal ID from name (snake_case, max 40 chars)
    ...
    // Export format matches src/data/scenarios/*.ts structure
    const exportText = `export const ${scenarioId} = ${JSON.stringify(cleanState, null, 2)};`;
    navigator.clipboard.writeText(exportText).then(() => {
      showToast(`Scenario "${scenarioId}" copied! Add to src/data/scenarios/`);
    });
  };
          ...
          <div className="nav-content">
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              >
                ...
                <span className="brand-text">CHOMBAT</span>
              </a>
            </div>
